...
void BSDegreeCalculator()
{
	...
	if (BS == 84)
	{
		cout << "MANS NOT HOT" << endl;
		PlaySound(TEXT("mansnothot"), NULL, SND_SYNC);
	}
	if (BS == 168)
	{
		cout << "Man won't take off his jacket." << endl;
		PlaySound(TEXT("jacket"), NULL, SND_SYNC);
	}
	if (BS == 252)
	{
		cout << "No Ketchup, Just Sauce, Raw Sauce" << endl;
		PlaySound(TEXT("rawsauce"), NULL, SND_SYNC);
	}
	if (BS == 336)
	{
		cout << "2 plus 2 is 4 minus 1 thats 3 quick maths" << endl;
		PlaySound(TEXT("qwickmaths"), NULL, SND_SYNC);
	}
	if (BS == 420)
	{
		cout << "The girl told me, \"Take off your jacket\" I said, \"Babes, man's not hot\"";
		PlaySound(TEXT("gunsounds"), NULL, SND_SYNC);
	}
